...
function buildSelection(selection: Selection, variables: RecordOfValues): APISelection | undefined {
  return buildFieldSelection(selection, variables) ||
    buildFragmentSelection(selection.getDefinition(), variables);
}

export const buildSelections = (selectionSet: Selection[], variables: RecordOfValues): Selections =>
  selectionSet.length > 0 ? selectionSet.map(sel => buildSelection(sel, variables)).filter(sel => !!sel) : undefined;