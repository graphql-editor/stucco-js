...
function setParentType(v: { setParenttype(tr: TypeRef): void }): void {
...
const setProtocol = (req: FieldResolveRequest): FieldResolveRequest =>
  req.setProtocol(
    objValue({
      headers: objValue({
...
function setSource(v: { setSource: (source: Value) => void }): void {
  v.setSource(
    objValue({
      some: objValue({
        source: arrValue([
